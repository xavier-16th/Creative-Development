...
function drawLightning(x, y, segments = 20) {
  ...
  for (let i = 0; i < segments; i++) {
    ...
    if (Math.random() < 0.2) {
      ctx.moveTo(ex, ey);
      ctx.lineTo(ex + (Math.random() - 0.5) * 30, ey + Math.random() * 20);
    }
...
function animate() {
  ...
  if (Math.random() < 0.02) {
    ...
    drawLightning(Math.random() * canvas.width, 0, Math.random() * 30 + 10);
  }
...
const angerSection = document.querySelector("#anger-page");
const starWarsAudio = document.querySelector("#star-wars-audio");
const starWarsPlayButton = document.querySelector("#star-wars-play");
const loveCrawl = document.querySelector(".crawl-love");
const desireCrawl = document.querySelector(".crawl-desire");
...
if (starWarsSection) {
  const observer = new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting) {
        starWarsSection.classList.add("is-active");
        if (starWarsAudio) {
          starWarsAudio.currentTime = 0;
          starWarsAudio.play().catch(() => {});
        }
      }
    },
    { threshold: 0.5 },
  ...
  observer.observe(starWarsSection);
}

if (starWarsAudio && starWarsPlayButton) {
  starWarsPlayButton.addEventListener("click", () => {
    starWarsAudio.currentTime = 0;
    starWarsAudio.play().catch(() => {});
  });
}
...
if (angerSection) {
  const observer = new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting) {
        angerSection.classList.add("is-active");
      }
    },
    { threshold: 0.5 },
  );

  observer.observe(angerSection);
}

if (loveCrawl && loveSection) {
  loveCrawl.addEventListener(
    "animationend",
    (event) => {
      if (event.animationName === "fade-out") {
        loveSection.classList.add("show-gif");
      }
    },
    { once: true },
  );
...
if (desireCrawl && desireSection) {
  desireCrawl.addEventListener(
    "animationend",
    (event) => {
      if (event.animationName === "fade-out") {
        desireSection.classList.add("show-gif");
      }
    },
    { once: true },
  );
